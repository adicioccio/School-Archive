...
/* External objects */
extern abr_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(abr_nul);
 ...
 * -------------------------------------------------------------
 */
abr_nul printScannerError(abr_chr* fmt, ...);
abr_nul displayScanner(Buffer* ptrBuffer);
abr_lng getScannerFilesize(abr_chr* fname);
abr_nul printToken(Token t);
