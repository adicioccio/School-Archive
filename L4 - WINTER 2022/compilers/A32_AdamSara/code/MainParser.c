...
abr_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern abr_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern abr_int line; /* source code line number - defined in scanner.c */
...
extern abr_nul startParser(void);
extern abr_int startScanner(BufferPointer sc_buf);

static abr_nul printParserError(char* fmt, ...);
static abr_nul displayParser(BufferPointer ptrBuffer);
static abr_lng getParserFileSize(char* fname);
static abr_nul callGarbageCollector(void);
...
abr_nul printParserError(abr_chr* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(abr_nul)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
abr_lng getParserFileSize(abr_chr* fname) {
	FILE* input;
	abr_lng flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
abr_nul displayParser(Buffer* ptrBuffer) {
...
abr_nul callGarbageCollector(abr_nul) {
