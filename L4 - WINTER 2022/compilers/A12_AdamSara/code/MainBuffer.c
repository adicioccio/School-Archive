 ...
 * -------------------------------------------------------------
 */
abr_null bErrorPrint(abr_chr* fmt, ...);
abr_null displayBuffer(Buffer* ptr_Buffer);
abr_lng getFileSize(abr_chr* fname);
abr_shrt isNumber(const abr_chr* ns);
abr_null startBuffer(abr_chr*, abr_chr*, abr_chr, abr_shrt, abr_shrt);
...
abr_shrt mainBuffer(abr_shrt argc, abr_chr** argv) {

	/* Create source input buffer */
	abr_chr* program = argv[0];
	abr_chr* input = argv[2];
	abr_chr mode = MODE_FIXED;
	abr_shrt isAnsiC = !ANSI_C;		/* ANSI C flag */
	abr_shrt size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
void startBuffer(abr_chr* program, abr_chr* input, abr_chr mode, abr_shrt size, abr_shrt increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	abr_shrt loadSize = 0;			/* the size of the file loaded in the buffer */
	abr_chr symbol;				/* symbol read from input file */
...
abr_null bErrorPrint(abr_chr* fmt, ...) {
	...
	(abr_null)vfprintf(stderr, fmt, ap);
	va_end(ap);
